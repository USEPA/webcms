 ...
 * Used for preserving allowed attributes on the Drupal Inline Media model.
 *
...
function modelToEditingViewAttributeConverter() {
  return (dispatcher) => {
    dispatcher.on(
      'attribute:linkHref:drupalInlineMedia',
      (evt, data, conversionApi) => {
        if (
          !conversionApi.consumable.consume(
            data.item,
            'attribute:htmlLinkAttributes:drupalInlineMedia',
          )
        ) {
          return;
        }
...
function modelToDataViewAttributeConverter() {
  return (dispatcher) => {
    ...
    dispatcher.on(
      'attribute:htmlAttributes:drupalInlineMedia',
 ...
 * @private
 */
export default class DrupalInlineMediaGeneralHtmlSupport extends Plugin {
  /**
   * @inheritdoc
   */
  constructor(editor) {
    ...
    dataSchema.registerBlockElement({
      ...
      view: 'drupal-inline-media',
    });

    dataFilter.on('register:drupal-inline-media', (evt, definition) => {
      if (definition.model !== 'drupalInlineMedia') {
        return;
      }

      schema.extend('drupalInlineMedia', {
  ...
  static get pluginName() {
    return 'DrupalInlineMediaGeneralHtmlSupport';
  }
}

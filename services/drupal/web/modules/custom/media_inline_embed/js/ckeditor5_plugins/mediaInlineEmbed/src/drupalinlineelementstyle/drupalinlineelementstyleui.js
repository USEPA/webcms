...
import DrupalInlineElementStyleEditing from './drupalinlineelementstyleediting';
...
function getUIComponentName(name, group) {
  return `drupalInlineElementStyle:${group}:${name}`;
}
 ...
 * @private
 */
export default class DrupalInlineElementStyleUi extends Plugin {
  /**
   * @inheritdoc
   */
  static get requires() {
  ...
  init() {
    ...
    const toolbarConfig = this.editor.config.get('drupalInlineMedia.toolbar') || [];
    const definedStyles = plugins.get(
      'DrupalInlineElementStyleEditing',
    ).normalizedStyles;
  ...
  _createButton(buttonConfig, group, definedStyles) {
    ...
    this.editor.ui.componentFactory.add(
      ...
      (locale) => {
        const command = this.editor.commands.get('drupalInlineElementStyle');
  ...
  _executeCommand(name, group) {
    this.editor.execute('drupalInlineElementStyle', {
      value: name,
      group,
    });
  ...
  static get pluginName() {
    return 'DrupalInlineElementStyleUi';
  }
}

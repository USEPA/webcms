...
function modelToViewStyleAttribute(styles) {
  return (evt, data, conversionApi) => {
    if (data.item.name != 'drupalInlineMedia') {
      return;
    }
...
function viewToModelStyleAttribute(styles, modelAttribute) {
  ...
  return (evt, data, conversionApi) => {
    ...
    if (modelElement.name !== 'drupalInlineMedia') {
      return;
    }
 ...
 *
 * Additional Drupal Element Styles can be defined with `drupalInlineElementStyles`
 * configuration key.
 *
 ...
 * @private
 */
export default class DrupalInlineElementStyleEditing extends Plugin {
  /**
   * @inheritdoc
   */
  init() {
    ...
    const stylesConfig = editor.config.get('drupalInlineElementStyles');
      ...
      this.normalizedStyles[group] = stylesConfig[group] // array of styles
        ...
        .filter((style) => {
          if (
            ...
            console.warn(
              `${style.attributeValue} drupalInlineElementStyles options must include attributeName and attributeValue.`,
            );
            return false;
          }
          if (!style.modelElements || !Array.isArray(style.modelElements)) {
            console.warn(
              'drupalInlineElementStyles options must include an array of supported modelElements.',
            );
            return false;
          }

          if (!style.name) {
            console.warn('drupalInlineElementStyles options must include a name.');
            return false;
          }
    ...
    editor.commands.add(
      ...
      new DrupalInlineElementStyleCommand(editor, this.normalizedStyles),
    );
  }
  ...
  static get pluginName() {
    return 'DrupalInlineElementStyleEditing';
  }
}

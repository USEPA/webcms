...
function createDrupalInlineMedia(writer, attributes) {
  const drupalInlineMedia = writer.createElement('drupalInlineMedia', attributes);
  return drupalInlineMedia;
}
 ...
 *
 * @example
 *    editor.execute('insertDrupalInlineMedia', {
 *      'alt': 'Alt text',
 *      'data-align': 'left',
 ...
 * @private
 */
export default class InsertDrupalInlineMediaCommand extends Command {
  execute(attributes) {
    const mediaEditing = this.editor.plugins.get('DrupalInlineMediaEditing');
    ...
    this.editor.model.change((writer) => {
      this.editor.model.insertContent(
        createDrupalInlineMedia(writer, modelAttributes),
      );
    });
  }

...
exec bash -e - <<'EOF'
echo "========================"
echo "SET LATEST REVISION"
echo "========================"
echo "Starting upgrade_d7_node_latest_revision"
drush migrate-import upgrade_d7_node_latest_revision --continue-on-failure;

# Check if unproccessed items is <= 0
drush_output=$(drush ms upgrade_d7_node_latest_revision --format string);
output=( $drush_output )
if [ "${output[9]}" == "0" ] || [ "${output[9]}" -lt "0" ]
  then
    echo "All expected upgrade_d7_node_latest_revision items imported.";
  else
    echo "Not all upgrade_d7_node_latest_revision items were imported. Stopping the migration.";
    exit 1;
  fi
EOF
SCRIPT

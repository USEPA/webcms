...
resource "aws_appautoscaling_scheduled_action" "business_hours" {
  ...
  scalable_target_action {
    ...
    min_capacity = (local.business_hours_min_tasks <= var.drupal_max_capacity && aws_ecs_service.drupal.name == "webcms-production-prod-en-drupal" ? local.business_hours_min_tasks : var.drupal_min_capacity)
    max_capacity = var.drupal_max_capacity
  }
}
